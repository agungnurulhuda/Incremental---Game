...
public class GameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for(int i = 0; i < 4; i++)
        {
            CoinIcon[NoCoin].transform.localScale = Vector3.LerpUnclamped(CoinIcon[NoCoin].transform.localScale, Vector3.one * 4f, 0.15f);
            //CoinIcon.transform.Rotate(0f, 0f, Time.deltaTime * -100f);
        }

        if(NoCoin == 0)
        {
            CoinIcon[0].gameObject.SetActive(true);
            CoinIcon[1].gameObject.SetActive(false);
            CoinIcon[2].gameObject.SetActive(false);
            CoinIcon[3].gameObject.SetActive(false);
        }else if (NoCoin == 1)
        {
            CoinIcon[0].gameObject.SetActive(false);
            CoinIcon[1].gameObject.SetActive(true);
            CoinIcon[2].gameObject.SetActive(false);
            CoinIcon[3].gameObject.SetActive(false);
        }
        else if (NoCoin == 2)
        {
            CoinIcon[0].gameObject.SetActive(false);
            CoinIcon[1].gameObject.SetActive(false);
            CoinIcon[2].gameObject.SetActive(true);
            CoinIcon[3].gameObject.SetActive(false);
        }
        else if (NoCoin == 3)
        {
            ...
            CoinIcon[1].gameObject.SetActive(false);
            CoinIcon[2].gameObject.SetActive(false);
            CoinIcon[3].gameObject.SetActive(true);
        }
    ...
    public void CollectByTap(Vector3 tapPosition, Transform parent)
    {
        ...
        CoinIcon[NoCoin].transform.localScale = Vector3.one * 1.75f;
